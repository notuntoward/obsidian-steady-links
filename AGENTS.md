...
`.obsidian/plugins/steady-links` points to the main checkout
(`~/repos/obsidian-steady-links/`), so Obsidian continues to load the
...
1. **Print the exact filesystem path of the built `main.js`** before
   ...
   > Reload Steady Links in Obsidian. The fresh `main.js` is at:
   > `C:\Users\scott\repos\obsidian-steady-links\.kilo\worktrees\foo\main.js`
...
2. **Unit-test and browser-test from the worktree as normal** (Vitest
   ...
   - **Re-linking the vault symlink** to the worktree:
     ```powershell
     Remove-Item -LiteralPath "$env:USERPROFILE\...\.obsidian\plugins\steady-links"
     New-Item -ItemType Junction `
              -Path "$env:USERPROFILE\...\.obsidian\plugins\steady-links" `
              -Target "$env:USERPROFILE\repos\obsidian-steady-links\.kilo\worktrees\<name>"
     ```
