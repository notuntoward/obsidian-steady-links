...
The first time you run the browser tests on a machine, install the Playwright Chromium browser:
...
- `npm run test:run` runs the Vitest unit and integration suite.
- `npm run test:browser` runs the Playwright browser regression tests against a real browser engine.

What each test layer covers:

- `Vitest` covers parsing, cursor movement logic, transaction filters, modal behavior, and other fast logic-level checks.
- `Playwright` covers layout-sensitive behavior that jsdom cannot reliably catch, such as the regression where focusing a logical line with a link shifts that line and the lines below it.

Recommended workflow when changing link-hiding behavior:

1. Run `npm run test:run` while iterating on logic.
2. Run `npm run test:browser` before merging changes that touch `src/linkSyntaxHider.ts`, cursor behavior, or hidden-syntax styling.

Notes:

- The Playwright tests use a small CodeMirror harness in `tests/playwright/` instead of launching the full Obsidian app, which keeps the browser regression suite focused and relatively fast.
- If the browser suite fails on a fresh machine, the usual cause is that Chromium has not been installed yet with `npx playwright install chromium`.
